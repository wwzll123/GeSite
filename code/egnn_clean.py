...
if __name__ == "__main__":
    ...
    # Initialize EGNN
    egnn = EGNN(in_node_nf=n_feat, hidden_nf=512, out_node_nf=2, in_edge_nf=1)
    egnn=egnn.half().to(device)
    h=h.half().to(device)
    x=x.half().to(device)
    edges[0],edges[1]=edges[0].to(device),edges[1].to(device)
    edge_attr=edge_attr.half().to(device)

    # Run EGNN
    # h是节点特征,x是节点的坐标矩阵,edges是边,edge_attr是边权重\
